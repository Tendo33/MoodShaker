...
export default function CocktailRecommendation() {
	...
	useEffect(() => {
		...
		// 开始轮询获取鸡尾酒图片
		if (cocktail && !imageData) {
			startImagePolling();
		}
	}, [cocktail, imageData, loadSavedData, startImagePolling]);
	...
	return (
		<div className={`min-h-screen transition-colors duration-300 ${themeClasses}`}>
			...
			<div className="container py-12 relative">
				...
				<div className={isLoading ? "flex items-center justify-center min-h-[50vh]" : "hidden"}>
					<div className="text-center">
						...
						<p className="text-gray-300 font-medium">正在为您准备鸡尾酒...</p>
					</div>
				</div>
				...
				<div
					className={
						!isLoading && !cocktail
							? "text-center py-12 backdrop-blur-sm rounded-2xl border border-white/10 shadow-xl"
							: "hidden"
					}
				>
					<h2 className={`text-2xl font-medium mb-4 ${textColorClass}`}>未找到鸡尾酒推荐</h2>
					<p className="text-gray-300 mb-6">抱歉，我们无法为您生成鸡尾酒推荐</p>
					<button
						onClick={handleBack}
				...
				<div
					className={
						!isLoading && cocktail ? "transition-all duration-500 transform translate-y-0 opacity-100" : "hidden"
					}
				>
					<div
						className={
					...
					<div
						className={
							cocktail
								? `mt-10 backdrop-blur-sm border border-white/10 shadow-xl rounded-2xl overflow-hidden transition-colors duration-300 ${cardClasses}`
								: "hidden"
						}
					>
					...
					<div className="mt-12 flex flex-col sm:flex-row gap-4 justify-center">
						...
						<button className="flex items-center bg-gradient-to-r from-amber-500 to-pink-500 hover:from-amber-600 hover:to-pink-600 border-0 text-white shadow-lg shadow-pink-500/20 px-6 py-3 rounded-full transition-all duration-300 hover:scale-105 whitespace-nowrap">
							<ThumbsUp className="mr-2 h-5 w-5" /> 喜欢这个推荐
						</button>
					</div>
				</div>
			</div>
